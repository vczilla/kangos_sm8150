...
bool _debug_pagealloc_enabled __read_mostly = IS_ENABLED(CONFIG_DEBUG_PAGEALLOC_ENABLE_DEFAULT);
								...
								unsigned long start_pfn, enum memmap_context context)
{
	...
	for (pfn = start_pfn; pfn < end_pfn; pfn++)
	{
		...
		if (mirrored_kernelcore && zone == ZONE_MOVABLE)
		{
			...
			{
				for_each_memblock(memory, tmp) if (pfn < memblock_region_memory_end_pfn(tmp)) break;
				r = tmp;
			}
